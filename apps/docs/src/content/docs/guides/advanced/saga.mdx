...
const fulfillOrder = definePipeline<FulfillCtx>('fulfill-order', () => {
  ...
  node('reserve-inventory', {
    action: async (ctx) => {
      // In a real system: call inventory service
      return { inventoryReserved: true };
    },
    forks: [
      fork((ctx) => ctx.inventoryReserved, 'charge-card',          { label: 'reserved' }),
      fork(undefined,                       'saga-failed-no-stock', { label: 'no-stock' }),
    ],
  });
  ...
  node('charge-card', {
    action: async (ctx) => {
      // In a real system: call payment processor
      const chargeId = `ch_${ctx.orderId}`;
      return { chargeId };
    },
    forks: [
      fork((ctx) => ctx.chargeId !== null, 'ship-order',                  { label: 'charged' }),
      fork(undefined,                       'compensate-release-inventory', { label: 'charge-failed' }),
    ],
  });
  ...
  node('ship-order', {
    action: async (ctx) => {
      // Simulate a transient failure on first two attempts
      if (ctx.shipAttempts < 2) {
        return { shipAttempts: ctx.shipAttempts + 1, shipped: false };
      }
      return { shipped: true };
    },
    forks: [
      fork((ctx) => ctx.shipped,              'saga-complete',              { label: 'shipped' }),
      // Retry: route back to ship-order while we haven't hit the limit
      fork((ctx) => ctx.shipAttempts < 3,     'ship-order',                 { label: 'retry' }),
      // Give up: compensate both prior steps
      fork(undefined,                          'compensate-refund-and-release', { label: 'shipping-failed' }),
    ],
  });
  ...
  node('compensate-release-inventory', {
    action: async (ctx) => {
      // In a real system: release inventory reservation
      return { inventoryReserved: false, error: 'charge-failed' };
    },
    forks: [fork(undefined, 'saga-compensated')],
  });

  // ── Compensation: refund + release (charge succeeded, shipping failed) ───────
  node('compensate-refund-and-release', {
    action: async (ctx) => {
      // In a real system: void ctx.chargeId, release inventory
      return { chargeId: null, inventoryReserved: false, error: 'ship-failed' };
    },
    forks: [fork(undefined, 'saga-compensated')],
  });
...
const order = defineDomain<OrderCtx>('order', () => {
  state('pending', {
    edges: {
      FULFILL: edge({
        target: 'fulfilled',
        guard: async (ctx) => ctx.chargeId === null, // not already charged
        pipeline: fulfillOrder,
        action: async (ctx) => {
          // Only transition if saga completed successfully
          if (!ctx.shipped) throw new Error('Saga did not complete');
          return {};
        },
      }),
    },
