...
Plexis never mutates context. Every handler that can modify state does so by returning a *patch* — a partial object that the runtime merges over the previous context. Understanding this model makes every `action`, `onEnter`, `onExit`, and node action predictable.
