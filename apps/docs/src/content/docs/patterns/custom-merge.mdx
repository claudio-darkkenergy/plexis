...
const order = defineDomain<Ctx>('order', () => {
  state('pending', {
    edges: {
      PAY: edge({
        target: 'paid',
        // Patch only sets paid — but the default merge shallow-copies the top level,
        // so flags: { paid: true } REPLACES the entire flags object.
        // { shipped: false } is lost!
        action: async () => ({ flags: { paid: true } }),
      }),
    },
  });
...
The third argument to `merge` is a `MergeMetadata` object with fields like `phase`, `domainId`, `pipelineId`, `stateId`, and `event`. Use it to apply different merge strategies per phase if needed — for example, deep merge only during `onEnter` but shallow merge on edge actions.
