...
A guard lives on an edge (`EdgeDef.guard`) and is evaluated before any side effects run. If it returns `false`, the event is blocked: no `exit`, no action, no pipeline, no state change, no history entry. The domain stays exactly where it was.
