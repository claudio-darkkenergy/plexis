...
plexis/
  ...
  .github/
    workflows/       ← build.yml, publish.yml, docs.yml, smoke-test.yml
```
...
| `docs.yml` | Pull request | Builds the Astro docs site to catch MDX errors |
...
CI runs only on pull requests (for development) and on `v*.*.*` tags (for releases). Plain branch pushes trigger no workflow runs.
