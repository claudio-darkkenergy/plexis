...
- **THEN** a page renders documenting the complete public surface of `@tde.io/plexis`: definition functions, registration helpers, handler input types, tracer, domain and pipeline instance members (including graph query APIs), result types, and supporting types
- **AND** every function and method documents its parameters and return value under clearly separated sections
- **AND** parameters and field names are visually emphasized so they stand out from type and description text
