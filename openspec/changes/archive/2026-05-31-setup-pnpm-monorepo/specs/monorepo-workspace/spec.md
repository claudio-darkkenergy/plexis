...
The root `package.json` SHALL declare pass-through scripts that delegate workspace-wide build/test/typecheck operations to the library package using `pnpm --filter @tde.io/plexis`. At minimum, the root MUST declare `build`, `test`, and `typecheck` scripts whose commands invoke the corresponding script in `@tde.io/plexis`.
...
- **WHEN** a developer inspects `packages/plexis/`
- **THEN** the files `tsconfig.json`, `tsconfig.types.json`, `rolldown.config.ts`, `vitest.config.ts`, and `vite.config.ts` exist inside the package directory
...
The repository SHALL contain an `apps/` directory at the root. The `apps/` directory SHALL be tracked by Git even when empty. Any package added under `apps/` MUST be marked private (`"private": true` in its `package.json`) and MUST NOT be a publish target.
...
#### Scenario: apps/docs/ slot is reserved

- **WHEN** a developer inspects `apps/`
- **THEN** a `docs/` subdirectory exists (initially containing only a `.gitkeep` or equivalent placeholder)

#### Scenario: Any future app under apps/ is private

- **WHEN** a developer adds a `package.json` to any `apps/*` subdirectory
- **THEN** that manifest contains `"private": true`

### Requirement: Single shared node_modules via pnpm

The workspace SHALL share dependencies through pnpm's content-addressed store. The repository SHALL NOT use `npm` or `yarn` to install workspace dependencies. The `packageManager` field in the root `package.json` SHALL pin pnpm to a specific version.
...
#### Scenario: pnpm install populates node_modules at root and per-package

- **WHEN** a developer runs `pnpm install` at the repository root
- **THEN** a `node_modules/` directory is created at the repository root
- **AND** a `packages/plexis/node_modules/` directory is created with pnpm-managed symlinks
